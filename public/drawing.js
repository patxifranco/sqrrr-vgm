...
const log = logger.scope('Drawing');
  ...
  function handleCorrectGuess(data) {
    ...
    // Play supersonic sound effect for everyone on correct guess
    const correctSound = new Audio('supersonic.mp3');
  ...
  function showResults(rankings) {
    ...
    if (elements.resultsRankings) {
      elements.resultsRankings.innerHTML = rankings.map((p, i) => `
        <div class="results-rank-item ${i === 0 ? 'winner' : ''}">
  ...
  function handleMouseUp(e) {
    ...
    if (currentTool === 'rectangle') {
      ...
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      ctx.drawImage(tempCanvas, 0, 0);
      drawShape(shapeStartX, shapeStartY, x, y, 'rectangle', currentColor, currentSize, true);
    }
  ...
  function floodFill(startX, startY, fillColor, send = false) {
    ...
    // Tolerance for color matching (handles anti-aliasing and slight variations)
    const tolerance = 32;
