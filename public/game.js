...
import { vgmChat, replaceEmoticons, emoticonMap } from './js/games/vgm/chat.js';
...
// Emoticons imported from vgm/chat.js module (replaceEmoticons, emoticonMap)
...
function showScreen(screenName) {
  ...
  if (previousScreen === 'game' && screenName !== 'game') {
    ...
    log.info('Left VGM game, cleaned up timers and socket listeners');
  }
...
function updatePlayerList(players, listElement) {
  ...
  players
    ...
    .forEach(player => {
      ...
      // Build streak HTML if player has a streak (Racha with capital R, to the right of score)
...
function startTimer(duration) {
  ...
  TimerManager.setInterval('vgm-round-timer', () => {
    ...
    updateProgressSegments(Math.min(100, progressPercent));
...
function startFadeOut() {
  // Calculate step size for 3-second fadeout
  // 3000ms / 100ms interval = 30 steps
...
// Close player profile popup
popupProfileClose.addEventListener('click', () => {
  playerProfilePopup.classList.remove('active');
});

// Close popup on outside click
playerProfilePopup.addEventListener('click', (e) => {
  if (e.target === playerProfilePopup) {
    playerProfilePopup.classList.remove('active');
  }
});
...
if (voteExtendBtn) {
  voteExtendBtn.addEventListener('click', () => {
    if (!hasVotedExtend && !fadeOutStarted && roundActive) {
...
// Lobby created
socket.on('lobbyCreated', ({ roomCode, playerName }) => {
...
// Lobby joined
socket.on('lobbyJoined', ({ roomCode, playerName }) => {
  currentRoom = roomCode;
  roomCodeDisplay.textContent = roomCode;
  gameRoomCode.textContent = roomCode;
  showScreen('lobby');
...
socket.on('vgmJoined', ({ roomCode, playerName, roundActive: isRoundActive, autoPlayActive: isAutoPlayActive, roundNumber: currentRoundNum, currentAudioToken, roundStartTime: serverRoundStartTime, roundDuration: serverRoundDuration }) => {
  currentRoom = roomCode;
...
// Chat message
socket.on('chatMessage', ({ system, message }) => {
  addMessage(chatMessages, message, system ? 'system-message' : '');
});

// Round start
socket.on('roundStart', ({ roundNumber: num, audioToken, duration }) => {
...
// Guess result
socket.on('guessResult', ({ correct, type, sonicType, timeElapsed }) => {
  if (correct) {
...
socket.on('correctGuess', ({ playerName, type, sonicType, timeElapsed }) => {
...
socket.on('hintResult', ({ success, hint, hintPoints: newPoints, reason }) => {
  if (success) {
...
// Someone used a hint
socket.on('playerUsedHint', ({ playerName }) => {
  if (currentUser && playerName !== currentUser.username) {
...
socket.on('roundEnd', ({ correctGame, correctSong, players }) => {
...
socket.on('sqrrrMessage', ({ message, isBold, isRecord }) => {
  if (isRecord) {
...
socket.on('sqrrrCountdown', ({ id, message }) => {
...
socket.on('gameChatMessage', ({ sender, message, profilePicture, fontSettings }) => {
...
socket.on('closeGuess', ({ guess, type, percentage }) => {
  const div = document.createElement('div');
  div.className = 'chat-msg';
...
// Handle easter eggs
socket.on('easterEgg', ({ type, message }) => {
  const div = document.createElement('div');
  div.className = 'chat-msg';
...
socket.on('newRecord', ({ player, time, previousPlayer, previousTime }) => {
  const recordSound = new Audio('supersonic.mp3');
  recordSound.volume = 0.8;
  recordSound.play().catch(() => {});
});

socket.on('audioDurationUpdate', ({ duration }) => {
  fullAudioDuration = duration;
});

socket.on('extendVotesUpdate', ({ votes, needed, totalPlayers }) => {
  if (extendVotesDisplay) {
    extendVotesDisplay.textContent = `${votes}/${needed}`;
  }
});

socket.on('roundExtended', ({ newDuration }) => {
  isExtended = true;
...
if (titlebar && gameWindow) {
  ...
  titlebar.addEventListener('mousedown', (e) => {
    ...
    // Use actual top-left position from bounding rect
    dragStartLeft = rect.left;
    dragStartTop = rect.top;

    // Switch to direct positioning (remove center transform)
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    // Constrain to viewport - window can't go outside (using top-left coords)
...
// Allows resizing from all edges and corners using dedicated resize handle elements

const resizeHandles = gameWindow ? gameWindow.querySelectorAll('.resize-handle') : [];
...
// Attach mousedown listener to each resize handle
...
document.addEventListener('mousemove', (e) => {
  ...
  // Use actual left/top positions (not center-based)
  let newLeft = startLeft;
  let newTop = startTop;

  // Handle east edge - extend right side only, left stays fixed
  if (resizeEdge.indexOf('e') !== -1) {
    newWidth = Math.max(500, startWidth + dx);
  }
  // Handle west edge - extend left side only, right stays fixed
  ...
  // Handle south edge - extend bottom only, top stays fixed
  if (resizeEdge.indexOf('s') !== -1) {
    newHeight = Math.max(400, startHeight + dy);
  }
  // Handle north edge - extend top only, bottom stays fixed
...
// Fix focus issue when switching tabs - re-enable input when window regains focus
...
if (gameToolbar) {
  ...
  // Tienda button opens slot popup
  tiendaBtn.addEventListener('click', () => slotPopup.toggle());
}
