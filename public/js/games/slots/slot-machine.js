...
export class SlotMachine {
    ...
    this.container.innerHTML = `
      <div class="slot-cabinet">
        <div class="slot-top-bar">
          ...
          <button class="slot-shop-btn" title="Tienda">\u{1FA99}</button>
        </div>

        <div class="slot-body">
          <div class="slot-payout-panel">
            <div class="payout-title">PREMIOS</div>
            <div class="payout-item"><span class="payout-sym">${this.symbolEmojis.seven}${this.symbolEmojis.seven}${this.symbolEmojis.seven}</span><span class="payout-val">350 $qr</span></div>
            <div class="payout-item"><span class="payout-sym">${this.symbolEmojis.bar}${this.symbolEmojis.bar}${this.symbolEmojis.bar}</span><span class="payout-val">300 $qr</span></div>
            <div class="payout-item"><span class="payout-sym">${this.symbolEmojis.bell}${this.symbolEmojis.bell}${this.symbolEmojis.bell}</span><span class="payout-val">250 $qr</span></div>
            <div class="payout-item"><span class="payout-sym">${this.symbolEmojis.grape}${this.symbolEmojis.grape}${this.symbolEmojis.grape}</span><span class="payout-val">200 $qr</span></div>
            ...
            <div class="payout-item"><span class="payout-sym">${this.symbolEmojis.lemon}${this.symbolEmojis.lemon}${this.symbolEmojis.lemon}</span><span class="payout-val">100 $qr</span></div>
            <div class="payout-item"><span class="payout-sym">${this.symbolEmojis.cherry}${this.symbolEmojis.cherry}${this.symbolEmojis.cherry}</span><span class="payout-val">50 $qr</span></div>
          </div>
        ...
        <div class="slot-shop-overlay" style="display: none;">
          <div class="window shop-popup">
            <div class="title-bar">
              <div class="title-bar-text">Tienda</div>
              <div class="title-bar-controls">
                <button aria-label="Close" class="shop-close-btn"></button>
              </div>
            </div>
            <div class="window-body shop-body">
              <p class="shop-empty-msg">Aún no está :(((</p>
            </div>
          </div>
        </div>
      </div>
  ...
  bindEvents() {
    ...
    loanBtn.addEventListener('click', () => {
      this.socket.emit('slotsRequestLoan');
    });
    ...
    // Shop button
    const shopBtn = this.container.querySelector('.slot-shop-btn');
    shopBtn.addEventListener('click', () => this.showShopOverlay());

    // Shop close button
    const shopCloseBtn = this.container.querySelector('.shop-close-btn');
    shopCloseBtn.addEventListener('click', () => this.hideShopOverlay());
  ...
  showShopOverlay() {
    const overlay = this.container.querySelector('.slot-shop-overlay');
    if (overlay) overlay.style.display = 'flex';
  }
